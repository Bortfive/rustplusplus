...
module.exports = {
  ...
  async execute(rustplus, client, message) {
    if (!rustplus.isServerAvailable())
      return rustplus.deleteThisRustplusInstance();
...
async function messageBroadcastTeamMessage(rustplus, client, message) {
  ...
  if (steamId === rustplus.playerId) {
    ...
    const commandDelayMs =
      parseInt(rustplus.generalSettings.commandDelay) * 1000;
...
async function messageBroadcastEntityChangedSmartAlarm(
  ...
  if (active) {
    ...
    let voiceMessageText = `${server.alarms[entityId].name}: ${server.alarms[entityId].message}`;
    if (rustplus.guildId === "927624007577141278") {
      voiceMessageText = `¡CALVOS! NOS RAIDEAN ${server.alarms[entityId].name}`;
    }

    await DiscordVoice.sendDiscordVoiceMessage(
      rustplus.guildId,
      voiceMessageText,
      server.alarms[entityId].name,
    );
  }
...
async function messageBroadcastEntityChangedStorageMonitor(
  ...
  if (
    ...
    message.broadcast.entityChanged.payload.capacity ===
    ...
    if (server.storageMonitors[entityId].reachable) {
      if (
        info.entityInfo.payload.capacity ===
