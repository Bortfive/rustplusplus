...
class DiscordBot extends Discord.Client {
  constructor(props) {
    ...
    this.logger = new Logger(
      Path.join(__dirname, "..", "..", "logs/discordBot.log"),
      "default",
    );
    ...
    this.botIntl = null;
    this.enIntl = null;
    this.enMessages = JSON.parse(
      Fs.readFileSync(Path.join(__dirname, "..", "languages", "en.json")),
      "utf8",
    );
  ...
  loadDiscordEvents() {
    const eventFiles = Fs.readdirSync(
      Path.join(__dirname, "..", "discordEvents"),
    ).filter((file) => file.endsWith(".js"));
  ...
  build() {
    ...
    this.login(Config.discord.token)
      .then(() => console.log("Login successful1"))
      .catch((error) => {
          ...
          case 503:
            {
              this.log(
                this.intlGet(null, "errorCap"),
                this.intlGet(null, "serviceUnavailable", {
                  error: JSON.stringify(error),
                }),
                "error",
              );
  ...
  logInteraction(interaction, verifyId, type) {
    ...
    args["guild"] =
      `${interaction.member.guild.name} (${interaction.member.guild.id})`;
    ...
    this.log(
      ...
      this.intlGet(null, `${type}Interaction`, args),
    );
  }
  ...
  async setupGuild(guild) {
    ...
    let category = await require("../discordTools/SetupGuildCategory")(
      this,
      guild,
    );
  ...
  readGeneralSettingsTemplate() {
    return JSON.parse(
      Fs.readFileSync(
        Path.join(__dirname, "..", "templates/generalSettingsTemplate.json"),
        "utf8",
      ),
    );
  }
  ...
  createRustplusInstancesFromConfig() {
    ...
    files.forEach((file) => {
      ...
      if (
        ...
        instance.serverList.hasOwnProperty(instance.activeServer)
      ) {
  ...
  isJpgImageChanged(guildId, map) {
    return (
      JSON.stringify(this.rustplusMaps[guildId]) !==
      JSON.stringify(map.jpgImage)
    );
  }
  ...
  findAvailableGroupId(guildId, serverId) {
    ...
    while (true) {
      const randomNumber = Math.floor(Math.random() * 1000);
      if (
        !instance.serverList[serverId].switchGroups.hasOwnProperty(randomNumber)
      ) {
        return randomNumber;
      }
    }
  ...
  async updateBattlemetricsInstances() {
    ...
    for (const guild of this.guilds.cache) {
          ...
          if (bmInstance.lastUpdateSuccessful) {
            ...
            if (this.battlemetricsInstances.hasOwnProperty(bmInstance.id)) {
              if (!activeInstances.includes(bmInstance.id)) {
                ...
                await this.battlemetricsInstances[bmInstance.id].evaluation(
                  bmInstance.data,
                );
              }
            } else {
      ...
      for (const [trackerId, content] of Object.entries(instance.trackers)) {
        if (!activeInstances.includes(content.battlemetricsId)) {
          ...
          if (
            this.battlemetricsInstances.hasOwnProperty(content.battlemetricsId)
          ) {
            /* Update */
            await this.battlemetricsInstances[
              content.battlemetricsId
            ].evaluation();
          } else {
            /* Add */
    ...
    const remove = Object.keys(this.battlemetricsInstances).filter(
      (e) => !activeInstances.includes(e),
    );
  ...
  async validatePermissions(interaction) {
    ...
    if (
      ...
      const str = this.intlGet(interaction.guildId, "notPartOfRole", {
        role: role.name,
      });
