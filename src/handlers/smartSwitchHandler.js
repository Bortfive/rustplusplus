...
module.exports = {
  handler: async function (rustplus, client, time) {
    ...
    /* No more autoDayNightOnOff features */
  ...
  smartSwitchCommandHandler: async function (rustplus, client, command) {
    ...
    let active;
    if (
      command.startsWith(`${entityCommand} ${onEn}`) ||
      command.startsWith(`${entityCommand} ${onLang}`)
    ) {
      ...
      command === `${entityCommand} ${statusEn}` ||
      command === `${entityCommand} ${statusLang}`
    ) {
      const info = await rustplus.getEntityInfoAsync(entityId);
      if (!(await rustplus.isResponseValid(info))) {
    ...
    module.exports.smartSwitchCommandTurnOnOff(
      rustplus,
      client,
      entityId,
      active,
    ...
    rustplus.currentSwitchTimeouts[entityId] = setTimeout(async function () {
      const instance = client.getInstance(guildId);
      if (!instance.serverList[serverId].switches.hasOwnProperty(entityId))
        return;

      await module.exports.smartSwitchCommandTurnOnOff(
        rustplus,
        client,
        entityId,
        !active,
  ...
  smartSwitchCommandTurnOnOff: async function (
    rustplus,
    client,
    entityId,
    active,
