...
module.exports = {
  handler: async function (rustplus, client) {
    ...
    if (rustplus.smartAlarmIntervalCounter === 0) {
      const alarmsToProcess = Object.keys(instance.serverList[serverId].alarms);
      const promises = alarmsToProcess.map(async (entityId) => {
        instance = client.getInstance(guildId); // Refresh instance per alarm just in case
        try {
          const info = await rustplus.getEntityInfoAsync(entityId);
          if (!(await rustplus.isResponseValid(info))) {
            if (instance.serverList[serverId].alarms[entityId].reachable) {
              await DiscordMessages.sendSmartAlarmNotFoundMessage(
                guildId,
                serverId,
                entityId,
              );
          ...
          client.log(
            "Error",
            `Failed to contact smart alarm ${entityId} on server ${serverId}`,
            "error",
          );
        }
      });
  ...
  smartAlarmCommandHandler: function (rustplus, client, command) {
    ...
    const entityId = Object.keys(alarms).find(
      (e) => command === `${prefix}${alarms[e].command}`,
    );
    ...
    const timeSinceTriggerSeconds = Math.floor(
      (new Date() - lastTriggerDate) / 1000,
    );
