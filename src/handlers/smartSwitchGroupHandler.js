...
module.exports = {
  ...
  updateSwitchGroupIfContainSwitch: async function (
    client,
    guildId,
    serverId,
    switchId,
  ...
  TurnOnOffGroup: async function (
    ...
    for (const [entityId, content] of Object.entries(
      instance.serverList[serverId].switches,
    )) {
    ...
    const promises = actionSwitches.map(async (entityId) => {
      const prevActive =
        instance.serverList[serverId].switches[entityId].active;
  ...
  smartSwitchGroupCommandHandler: async function (rustplus, client, command) {
    ...
    let active;
    if (
      command.startsWith(`${groupCommand} ${onEn}`) ||
      command.startsWith(`${groupCommand} ${onLang}`)
    ) {
      active = true;
    } else if (
      command.startsWith(`${groupCommand} ${offEn}`) ||
      command.startsWith(`${groupCommand} ${offLang}`)
    ) {
      active = false;
    } else if (
      command === `${groupCommand} ${statusEn}` ||
      command === `${groupCommand} ${statusLang}`
    ) {
      const switchStatus = switchGroups[groupId].switches.map((switchId) => {
        const { active, name, reachable } =
          instance.serverList[serverId].switches[switchId];
      ...
      rustplus.sendInGameMessage(
        `${client.intlGet(guildId, "status")}: ${statusMessage}`,
      );
      return true;
    } else {
      return true;
    ...
    if (timeSeconds === null) {
      ...
      await module.exports.TurnOnOffGroup(
        client,
        rustplus,
        guildId,
        serverId,
