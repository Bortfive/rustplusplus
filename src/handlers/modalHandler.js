...
module.exports = async (client, interaction) => {
  ...
  if (interaction.customId.startsWith("CustomTimersEdit")) {
    ...
    const cargoShipEgressTime = parseInt(
      interaction.fields.getTextInputValue("CargoShipEgressTime"),
    );
    const oilRigCrateUnlockTime = parseInt(
      interaction.fields.getTextInputValue("OilRigCrateUnlockTime"),
    );
    ...
    if (
      cargoShipEgressTime &&
      cargoShipEgressTime * 1000 !== server.cargoShipEgressTimeMs
    ) {
      server.cargoShipEgressTimeMs = cargoShipEgressTime * 1000;
    }
    if (
      oilRigCrateUnlockTime &&
    ...
    const server = instance.serverList[ids.serverId];
    const battlemetricsId = interaction.fields.getTextInputValue(
      "ServerBattlemetricsId",
    );
    ...
    if (battlemetricsId !== server.battlemetricsId) {
      if (battlemetricsId === "") {
        ...
        client.battlemetricsInstances.hasOwnProperty(battlemetricsId)
      ) {
    ...
    const server = instance.serverList[ids.serverId];
    const smartSwitchName =
      interaction.fields.getTextInputValue("SmartSwitchName");
    const smartSwitchCommand =
      interaction.fields.getTextInputValue("SmartSwitchCommand");
    let smartSwitchProximity = null;
    try {
      smartSwitchProximity = parseInt(
        interaction.fields.getTextInputValue("SmartSwitchProximity"),
      );
    } catch (e) {
    ...
    if (
      ...
      !Keywords.getListOfUsedKeywords(client, guildId, ids.serverId).includes(
        smartSwitchCommand,
      )
    ) {
    ...
    const server = instance.serverList[ids.serverId];
    const switchId = interaction.fields.getTextInputValue(
      "GroupRemoveSwitchId",
    );
    ...
    server.switchGroups[ids.groupId].switches = server.switchGroups[
      ids.groupId
    ].switches.filter((e) => e !== switchId);
    client.setInstance(interaction.guildId, instance);
    ...
    const server = instance.serverList[ids.serverId];
    const smartAlarmName =
      interaction.fields.getTextInputValue("SmartAlarmName");
    const smartAlarmMessage =
      interaction.fields.getTextInputValue("SmartAlarmMessage");
    const smartAlarmCommand =
      interaction.fields.getTextInputValue("SmartAlarmCommand");
    ...
    const server = instance.serverList[ids.serverId];
    const storageMonitorName =
      interaction.fields.getTextInputValue("StorageMonitorName");
    ...
    const trackerBattlemetricsId = interaction.fields.getTextInputValue(
      "TrackerBattlemetricsId",
    );
    const trackerClanTag =
      interaction.fields.getTextInputValue("TrackerClanTag");
    ...
    if (trackerBattlemetricsId !== tracker.battlemetricsId) {
      if (
        client.battlemetricsInstances.hasOwnProperty(trackerBattlemetricsId)
      ) {
        const bmInstance =
          client.battlemetricsInstances[trackerBattlemetricsId];
    ...
    if (
      ...
      (!isSteamId64 &&
        tracker.players.some((e) => e.playerId === id && e.steamId === null))
    ) {
      interaction.deferUpdate();
      return;
    }
    ...
    if (isSteamId64) {
      ...
      tracker.players = tracker.players.filter(
