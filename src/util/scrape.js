...
module.exports = {
  ...
  scrapeSteamProfilePicture: async function (client, steamId) {
    const response = await module.exports.scrape(
      `${Constants.STEAM_PROFILES_URL}${steamId}`,
    );
  ...
  scrapeSteamIdFromVanity: async function (client, vanity) {
    ...
    const url = `https://steamcommunity.com/id/${encodeURIComponent(
      vanity.trim(),
    )}/?xml=1`;
    ...
    const steamId64Match = response.data.match(
    ...
    const profileLinkMatch = response.data.match(
